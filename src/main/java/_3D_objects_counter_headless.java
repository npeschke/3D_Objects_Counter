 ...
 * @version 1.0, 7/11/07
 */
public class _3D_objects_counter_headless implements PlugIn, AdjustmentListener, FocusListener {
    ImagePlus imp;
    ImageProcessor ip;
...
//        ((TextField)values.elementAt(1)).addFocusListener(this);
        ...
        gd.addStringField("Result Path", "./detected_blobs.csv");
...
//        gd.getNextNumber();
        resultPath = Paths.get(gd.getNextString());
        ...
        if (showStat) {
            ocResult = OC.showStatistics(newRT);
            ocResult.save(resultPath.toString());
        }
